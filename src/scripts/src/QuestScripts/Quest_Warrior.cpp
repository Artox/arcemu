...
#define SPAWNPOINT_TWIGGYFLATHEAD -1686.14, -4323.04, 4.28091
#define SPAWNPOINT_AFFRAYCHALLENGER -1679.3000488281f, -4328.9599609375f, 2.5859100818634f, 0.0f
#define SPAWNPOINT_BIGWILL -1705, -4344, 4.8, 1.2
...
public:
	...
	void OnLoad()
	{
		...
		sEAS.WaypointCreate(GetUnit(), -1698, -4344, 4.6, 0.2, 0, RUN, 0);
		sEAS.WaypointCreate(GetUnit(), -1684, -4335, 2.9, 0.7, 0, RUN, 0);
		sEAS.WaypointCreate(GetUnit(), -1683, -4329, 2.8, 0, 0, RUN, 0);
...
void AreaTriggerHook(Player *pPlayer, uint32 Id)
{
	...
	{
		Creature *twiggy = pPlayer->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(-1686.14, -4323.04, 4.28091, NPC_TWIGGYFLATHEAD);
		if(twiggy)
